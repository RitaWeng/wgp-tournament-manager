...
（下述為 P0 之前的原始問題描述；鎖定狀態對帳已由 P0/P0.1 補上、守衛可見化已由 P1 補上，見 §3）
三者之間原本沒有「以本機 `scoredRounds` 為權威、把後端鎖定狀態校正到一致」的對帳步驟。只要三條流程的狀態對不齊，就會出現黑洞。放大黑洞的兩個具體機制：
...
| **S5** | **發佈後本地改了配對但忘記重發** | 裁判手機顯示舊配對、照舊配對回報（HTTP 200）；主控端因選手對不上（守衛 B）**靜默略過**，雙方都不知道回報去哪 | 配對未對帳 ＋ 守衛 B | **✅ P1 已修**（2026-07-19） |
| **S6** | **線上賽事進行中按「重設」** | `wgpOnlineSync` 殘留、輪詢照跑、後端賽事與 QR 卡全部有效，但本機桌次已清空 → 回報無處可套。重設完全不碰線上狀態 | 生命週期未連動 | **✅ P2 已修**（2026-07-19） |
| **S7** | **鎖定競態**：裁判在鎖定前一刻送出更正，主控端輪詢在鎖定後才處理到 | 合法送出（HTTP 200）的更正被守衛 A 靜默丟棄，**兩端皆無提示** | 時序競態 ＋ 守衛 A | **✅ P1 已修**（2026-07-19） |
...
### P1 — 把「靜默丟棄」變成「可見事件」（S1/S2/S7 的止血）✅ 已實作（2026-07-19）

實作（`TournamentManager.tsx`）：新增 `droppedReports` 狀態（key `${round}-${table}` →
`DroppedReport`，含 reason `'locked'|'pairing'`、被丟棄版本與內容），**隨 appState 持久化**
（重載不消失）。守衛 A 丟棄「桌勝方與本機登錄不同」的新版本回報時寫入標記；
線上面板狀態列出現「⚠ 未採計回報：R… 桌…」連結、桌卡右上出現「⚠ 更正未採計」chip，
點擊開查看視窗（顯示更正內容與登錄差異）→ 一鍵「解除第 N 輪鎖定」（`doUnlockRound`，
自 `unlockRound` 抽出、免二次確認）→ 回報於下次輪詢**進入既有的採計確認流程**決定
採計或維持現狀，標記自動清除。裁判改回與登錄一致、或該輪重新發佈時，標記也自動清除。
→ S1/S2/S7 的黑洞「看得見、可處置」，且決策路徑完全重用既有確認窗，無新分支。

### P1 — 配對不符的偵測與提示（S5）✅ 已實作（2026-07-19）

- **主動偵測**：發佈成功時記各輪配對指紋 `publishedPairings`（隨 appState 持久化）；
  本地配對與指紋不符（`applyPairingEdit`／重抓對後未重發）→ 面板狀態列顯示
  「⚠ 配對已變更未重新發佈（R…）」。重新發佈或改回配對即解除。
- **被動止血**：守衛 B 丟棄配對不符的回報時寫入 `droppedReports`（reason `'pairing'`），
  桌卡出現「⚠ 配對不符」chip，查看視窗說明並指引重新發佈；重新發佈（或配對恢復一致、
  回報進入正常流程）後標記自動清除。「本地已無此桌／已變輪空」也視為配對不符留標記
  （對抗式 review 補漏：原實作在 `!match || bye` 就先 continue，重抓對縮桌數時仍靜默）。
- **對抗式 review（gpt-5.6-sol）另補三項**：① 匯入舊備份（缺新欄位）時 `judgeReports`／
  標記／指紋整組清空而非沿用現值（殘留高 version 會靜默擋掉匯入後的合法回報）；
  ② `publishEpochRef` 發佈世代——作廢發佈前已在途的輪詢回應，避免舊回報在發佈清檔後
  誤標配對不符或重新套用；③ retention／S7 e2e 測試時序穩健化。
  不採納兩項：鎖定中組明細不套用屬既定設計（解鎖後會補上、非永久遺失）；
  reset 殘留線上狀態即 S6 本體（P2）。

### P2 — 生命週期連動（S6）✅ 已實作（2026-07-19）

實作（`TournamentManager.tsx`）：`resetSystem` 改**兩層確認**（`dialog.confirm` 是二元的）——
第一層沿用既有重設確認、`onlineCfg` 存在時加註「線上賽事進行中，下一步將詢問如何處理」；
第二層「一併結束線上賽事？」：**建議路徑**＝`doCloseOnlineEvent(strict=false)`（自
`closeOnlineEvent` 抽出：關後端＋斷線；後端連不上也照樣斷線，殘留賽事由 7 天 retention
回收）；**保留路徑**＝`disconnectOnlineLocal()`（不關後端但清憑證停輪詢，訊息明確警告
「憑證只存本機、之後無法再管理」）。兩條路徑都必經斷線——同時堵住 Codex review 指出的
相鄰問題：重設清空 `scoredRounds` 後，殘留輪詢對帳會把後端已鎖輪次全部主動解開。
第二層 `danger:true` 不給 ESC/點背景關閉，避免誤觸「保留」。
e2e 新增 S6 回歸步驟（建新賽事→重設兩層確認→驗後端已刪 401、`wgpOnlineSync` 清除、
本機狀態歸零），共 27 步。
...
> 進度：S1–S4＋F1 回歸已在 `backend/test/e2e-online.mjs`（2026-07-18）；S7（鎖定競態
> 留標記→解鎖採計）與 S5（改配對未重發偵測＋配對不符標記→收斂）回歸已併入同一 e2e
> （2026-07-19，共 26 步）；S6（重設兩層確認→後端刪除＋本機歸零）亦已併入
> （2026-07-19 深夜，共 27 步）。**S1–S7 回歸全數落地。**
...
- **進度（2026-07-18）**：**P0.1 已實作並全數驗證 ✅**（實作 `fe8cd31`，無狀態鎖定對帳）——
  ...
  ≤4s 收斂屬可接受。詳 §7。
- **進度（2026-07-19）**：**P1（S7 守衛 A 可見化＋S5 配對對帳）已實作並驗證 ✅**——
  守衛 A/B 丟棄回報一律留可見標記（`droppedReports`，持久化），S7 可由標記一鍵解鎖後走
  既有採計確認；S5 主動偵測（發佈指紋 `publishedPairings`）＋被動標記。e2e 新增 S7 兩步
  ＋S5 三步全綠。
- **進度（2026-07-19 深夜）**：**S6（P2 重設防呆）已實作並驗證 ✅**——resetSystem 兩層
  確認（一併結束／保留但斷線），兩路徑皆斷線、一併堵住 Codex review 指出的「重設後
  對帳主動解鎖後端」相鄰問題（見 §3 P2）。e2e 27 步 ALL PASS。
  **至此 S1–S7 全部結案，本文件無殘留待辦。**
